...
public class Num82 {
    ...
    /**
     * 执行用时 : 1 ms , 在所有 java 提交中击败了 98.33% 的用户
     * 内存消耗 : 36.8 MB , 在所有 java 提交中击败了 60.58% 的用户
     *
    ...
    public static ListNode deleteDuplicates(ListNode head) {
        ...
        while (head!=null) {
            ...
            boolean flag = false;
            //删除重复节点（此处没有删第一个重复节点），并将标志有重复值的flag置为true
            while (head.next != null && head.val == head.next.val) {
                head.next = head.next.next;
                flag = true;
            }
            //如果flag=false表示之前没有遇到重复节点，则保留当前节点
            if (flag == false) {
                pre = head;
                head = head.next;
            } else if (head.next != null && head.val != head.next.val) {
                //如果flag=true且当前节点之后还有节点，则删除当前节点
                head.val = head.next.val;
                head.next = head.next.next;
            } else {
                //如果flag=true且当前节点是尾结点，则pre.next=null
                pre.next = null;
            }
        }
        return node.next;
