 ...
 *                               2.2 node是父节点的右子节点，则需要不断地找父节点，记为node=node.father，直到父节点不再是右子节点，而是左子节点，即node==node.father.left,下一节点就是node.father。
 *                               注意：2.1和2.2可以合为一种情况。
 */
public class InOrderNextNode {
    ...
    public TreeLinkNode GetNext(TreeLinkNode node) {
        ...
        //3.两种情况会跳出循环：
        //3.1.node.father==null，说明node是根节点，则下一节点就是null（=node.father）；
        //3.2.node.father.left=node，说明找到了node的父节点是左子节点的情况，则下一节点就是node.father。
        return node.father;
    }
